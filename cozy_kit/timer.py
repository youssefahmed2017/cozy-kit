...
class Timer:
    ...
    def stop_pomodoro(self) -> None:
        if not self.pomodoro_started:
            raise errors.PomodoroNotStartedError(
                "You didn't start the Pomodoro. "
                "Please start before you can stop it."
            )
    ...
    def wait(
        ...
        Parameters:
            ...
            time_type:
                sec, min, or hour.
        """
        sleep_count = self.__format_type(
            count=count,
            time_type=time_type,
        )
    ...
    def end_stopwatch(self):
        ...
        """
        # FIX 4: Guard must come BEFORE using self.start_time, not after.
        # Also reset stopwatch_running so it can be started again.
        if not self.stopwatch_running:
            raise errors.StopWatchNotStartedError(
                "Stopwatch not started! "
                "Please run start_stopwatch() first."
            )
